...
class BasicCNN(nn.Module):
    def __init__(self, num_channels, input_shape, name):
        ...
        self.fc1 = nn.Linear(in_features=6*conv_out_h *
                             conv_out_w, out_features=1)
...
class LongCNN(nn.Module):
    def __init__(self, num_channels, input_shape, name):
        ...
        pool1_hw = c2_hw[0] // 2, c2_hw[1] // 2

        self.conv3 = nn.Conv2d(
            in_channels=128, out_channels=256, kernel_size=3)
        c3_hw = conv_output_shape(*pool1_hw, 3)

        self.conv4 = nn.Conv2d(
            in_channels=256, out_channels=256, kernel_size=3)
        c4_hw = conv_output_shape(*c3_hw, 3)

        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        pool2_hw = c4_hw[0] // 2, c4_hw[1] // 2

        #self.conv5 = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=3)

        conv_out_h, conv_out_w = pool2_hw
        self.fc1 = nn.Linear(in_features=256*conv_out_h *
                             conv_out_w, out_features=1024)
        self.fc2 = nn.Linear(in_features=1024, out_features=1)
